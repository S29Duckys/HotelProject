...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3 text-sm text-slate-400 font-medium">
        ...
        <span class="text-slate-700 font-bold"><?= $solo[0]->getName() ?></span>
    </div>
    ...
    <div class="glass-card rounded-3xl p-8">
        <div class="flex items-start justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <h2 class="text-2xl font-black text-slate-900"><?= $solo[0]->getName() ?></h2>
                    <p class="text-sm text-slate-400 italic mt-1"><?= $solo[0]->getDescription() ?></p>
                </div>
            </div>
            <span class="text-3xl font-black text-blue-600 shrink-0"><?= $solo[0]->getPrixUn() ?> €</span>
        </div>
    </div>
    ...
    <div class="glass-card rounded-3xl p-8">
        ...
        <?php foreach ($solo as $restau) { ?>
            <div class="border border-slate-100 rounded-2xl p-5 hover:bg-slate-50 transition-colors">
                <div class="w-10 h-10 bg-emerald-500 text-white rounded-xl flex items-center justify-center font-black mb-3">R<?= $restau->getIdRestaurant() ?></div>
                <p class="font-bold text-slate-800 text-sm"><?= $restau->getName() ?></p>
            </div>
        <?php } ?>
